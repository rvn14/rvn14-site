...
import { FaAws, FaCss3Alt, FaDocker, FaGitAlt, FaGithub, FaHtml5, FaJava, FaJs, FaNodeJs, FaPython } from "react-icons/fa6"
import {
  SiAndroidstudio,
  SiCplusplus,
  SiDotnet,
...
const mobileLastRowStart = smallSkills.length - (smallSkills.length % 4 || 4)
const desktopLastRowStart = smallSkills.length - (smallSkills.length % 8 || 8)

const getSmallSkillCellClass = (index) => [
  "group flex items-center justify-center h-30 md:h-40 border-gray-300 hover:bg-black hover:text-lavender-100 transition-colors duration-500",
  (index + 1) % 4 !== 0 && index !== smallSkills.length - 1 ? "border-r" : "",
  (index + 1) % 8 !== 0 && index !== smallSkills.length - 1 ? "md:border-r" : "",
  index < mobileLastRowStart ? "border-b" : "",
  index < desktopLastRowStart ? "md:border-b" : "",
].filter(Boolean).join(" ")
  ...
  return (
    <section className="w-full bg-black mb-8 px-2  ">
      ...
      className="container abcont mx-auto p-2 md:p-4  bg-lavender-100 rounded-xl">
        ...
        <div className="mt-12">
          {/* Top grid: 4 core skills, responsive */}
          <div className="grid grid-cols-4 sm:grid-cols-2 md:grid-cols-4 border-b border-gray-300">
            
            <div className="group flex items-center justify-center h-30 md:h-56 border-r border-gray-300 p-4 md:p-16 hover:bg-black hover:text-lavender-100 transition-colors duration-500" aria-label="nextJS" title="nextJS">
          ...
          <div className="grid grid-cols-4 sm:grid-cols-4 md:grid-cols-8">
            {smallSkills.map(({ label, Icon }, index) => (
              <div key={label} className={getSmallSkillCellClass(index)} title={label} aria-label={label}>
                <Icon className="text-6xl text-black group-hover:text-lavender-100 transition-colors duration-500" />
              </div>
            ))}
          </div>
        </div>
